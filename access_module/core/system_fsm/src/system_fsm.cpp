...
portunus_err_t SystemFSM::init()
{
    ...
    if (m_reader != nullptr) {
        ...
        if (err == PORTUNUS_OK) {
            ...
            ESP_LOGW(TAG, "Credential reader init failed (0x%" PRIx32 ") — card polling disabled", err);
        }
    } else {
        m_caps.has_reader = false;
    ...
    if (m_access != nullptr) {
        ...
        if (err == PORTUNUS_OK) {
            ...
            ESP_LOGW(TAG, "Access point init failed (0x%" PRIx32 ") — door control disabled", err);
        }
    } else {
        m_caps.has_access_point = false;
    ...
    if (m_feedback != nullptr) {
        ...
        if (err == PORTUNUS_OK) {
            ...
            ESP_LOGW(TAG, "Feedback init failed (0x%" PRIx32 ") — LED disabled", err);
        }
    } else {
        m_caps.has_feedback = false;
    ...
    case EVENT_ACCESS_GRANTED: {
        ...
        if (m_caps.has_access_point) {
            ...
            if (err == PORTUNUS_OK) {
                ...
                ESP_LOGE(TAG, "Failed to unlock: 0x%" PRIx32, err);
            }
        } else {
